...
def array_mutation(array_, P, std):
...
def list_select_mutants(vect_select,P, std):
...
def completes_mutants(vect_select,mutants_select,P, std):
    ...
    if S<5 :
        ...
        for i in range (nb_missing_mut):
            new_mutant=array_mutation(vect_select[i%len(vect_select)],P, std)
...
def allNewvectors(vect_select,P,std):
    """_summary_ : function that returns the 10 new vectors (5 mutated + 5 crossing over) in a list in once (using all the previous functions)
        ...
        """
    mutated = mutatesAll(vect_select,P, std)
